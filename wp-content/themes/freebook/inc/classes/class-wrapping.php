 ...
 */
add_filter( 'template_include', array( 'FreeBook_Wrapping', 'wrap' ), 99 );
add_filter( 'freebook_wrap_base', 'freebook_wrap_base_cpts' );
 ...
 */
function freebook_template_path() {
	return FreeBook_Wrapping::$main_template;
}
 ...
 */
function freebook_template_base() {
	return FreeBook_Wrapping::$base;
}

if ( ! class_exists( 'FreeBook_Wrapping' ) ) {
	...
	class FreeBook_Wrapping {
		...
		/**
		 * Initialize new instance of FreeBook_Wrapping class by setting a variable for the $slug (which defaults to base)
		...
		public function __toString() {
			$this->templates = apply_filters( 'freebook_wrap_' . $this->slug, $this->templates );
		 ...
		 * and $base as static variables in FreeBook_Wrapping class.
		 *
		 * @since 1.0.0
 ...
 */
function freebook_wrap_base_cpts( $templates ) {
