 ...
 * @return array
 */
function freebook_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! freebook_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'freebook_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'freebook_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'freebook' ) . $network['name'],
 ...
 * @return string
 */
function freebook_get_single_share_buttons() {
	ob_start();
