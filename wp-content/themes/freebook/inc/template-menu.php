 ...
 * @return string
 */
function freebook_get_main_menu() {
	$args = apply_filters( 'freebook_main_menu_args', array(
		'theme_location'   => 'main',
		'container'        => '',
		'menu_id'          => 'main-menu',
		'echo'             => false,
		'fallback_cb'      => 'freebook_set_nav_menu',
		'fallback_message' => esc_html__( 'Set main menu', 'freebook' ),
	) );
 ...
 * @return void
 */
function freebook_main_menu() {

	$main_menu = freebook_get_main_menu();
 ...
 * @return void
 */
function freebook_vertical_main_menu( $slide = 'left', $args = array() ) {

	$default_args = apply_filters( 'freebook_vertical_menu_args', array(
		...
		'button-back-inner-html'  => '<i class="linearicon linearicon-chevron-left"></i><span class="navigation-button__text">' . esc_html__( 'Back', 'freebook' ) . '</span>',
		'button-close-inner-html' => '<i class="linearicon linearicon-cross"></i>',
	) );

	$args        = wp_parse_args( $args, $default_args );
	$menu        = freebook_get_main_menu();
 ...
 * @return void
 */
function freebook_footer_menu() {
	if ( ! get_theme_mod( 'footer_menu_visibility', freebook_theme()->customizer->get_default( 'footer_menu_visibility' ) ) ) {
		return;
	}

	$args = apply_filters( 'freebook_footer_menu_args', array(
		...
		'echo'             => false,
		'fallback_cb'      => 'freebook_set_nav_menu',
		'fallback_message' => esc_html__( 'Set footer menu', 'freebook' ),
	) );
 ...
 * @return void
 */
function freebook_top_menu() {
	
	if ( ! get_theme_mod( 'top_menu_visibility', freebook_theme()->customizer->get_default( 'top_menu_visibility' ) ) ) {
		return;
	}
	...
	wp_nav_menu( apply_filters( 'freebook_top_menu_args', array(
		'theme_location'  => 'top',
		'container'       => 'div',
 ...
 * @return string
 */
function freebook_get_social_list( $context, $type = 'icon' ) {
	static $instance = 0;
	$instance++;
	...
	$args = apply_filters( 'freebook_social_list_args', array(
		...
		'echo'             => false,
		'fallback_cb'      => 'freebook_set_nav_menu',
		'fallback_message' => esc_html__( 'Set social menu', 'freebook' ),
	), $context, $type );
 ...
 * @return null|string
 */
function freebook_set_nav_menu( $args ) {
 ...
 * @return void
 */
function freebook_menu_toggle( $menu_id ) {
	$format = apply_filters(
		'freebook_menu_toggle_html',
 ...
 * @return void
 */
function freebook_vertical_menu_toggle( $menu_id ) {
	$format = apply_filters(
		'freebook_vertical_menu_toggle_html',
