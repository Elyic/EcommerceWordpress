...
if ( ! class_exists( 'FreeBook_About_Widget' ) ) {

	/**
	 * Class FreeBook_About_Widget.
	 */
	class FreeBook_About_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display an information about your site.', 'freebook' );
			$this->widget_id          = 'freebook_widget_about';
			$this->widget_name        = esc_html__( 'About FreeBook', 'freebook' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => '',
				...
				'media_id' => array(
					...
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Upload', 'freebook' ),
					'value'              => '',
					'label'              => esc_html__( 'Logo:', 'freebook' ),
				),
				'enable_social' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'enable_social' => esc_html__( 'Enable Social Buttons', 'freebook' ),
					),
				),
				'enable_tagline' => array(
					...
					'options' => array(
						'enable_tagline' => esc_html__( 'Enable Tagline:', 'freebook' ),
					),
				),
				'content'  => array(
					'type'              => 'textarea',
					'placeholder'       => esc_html__( 'Text or HTML', 'freebook' ),
					'value'             => '',
					'label'             => esc_html__( 'Content:', 'freebook' ),
		...
		public function get_social_nav( $wrapper ) {
			...
			if ( is_array( $social_enabled ) && 'true' === $social_enabled['enable_social'] ) {
				$content = sprintf( $wrapper, freebook_get_social_list( 'widget' ) );
			}
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'freebook_widget_about_template_file', 'inc/widgets/about/views/about.php' ), false, false );
			...
			$src       = wp_get_attachment_image_src( $media_id, apply_filters( 'freebook_about_widget_image_size', 'medium' ) );
			...
			if ( $tagline_enabled ) {
				$format   = apply_filters( 'freebook_about_widget_tagline_format', '<p>%s</p>', $this->settings, $this->args );
...
add_action( 'widgets_init', 'freebook_register_about_widget' );
/**
 * Register about widget.
 */
function freebook_register_about_widget() {
	register_widget( 'FreeBook_About_Widget' );
}
