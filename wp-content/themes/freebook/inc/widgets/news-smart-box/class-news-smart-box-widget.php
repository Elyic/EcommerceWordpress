...
/**
 * Class FreeBook_News_Smart_Box_Widget
 */
class FreeBook_News_Smart_Box_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_id          = 'freebook_widget_news_smart_box';
		$this->widget_name        = esc_html__( 'News Smart Box', 'freebook' );
		$this->widget_description = esc_html__( 'News Smart Box', 'freebook' );
		$this->utility            = freebook_utility()->utility;
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'News Smart Box', 'freebook' ),
			...
			'layout_type' => array(
				...
				'options' => array(
					'layout_type_1' => array(
						'label'   => esc_html__( 'Layout type 1', 'freebook' ),
						'img_src' => FREEBOOK_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-1.svg',
					),
					'layout_type_2' => array(
						'label'   => esc_html__( 'Layout type 2', 'freebook' ),
						'img_src' => FREEBOOK_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-2.svg',
					),
					'layout_type_3' => array(
						'label'   => esc_html__( 'Layout type 3', 'freebook' ),
						'img_src' => FREEBOOK_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-3.svg',
					),
				),
				'label' => esc_html__( 'Choose layout type', 'freebook' ),
			),
			'terms_type' => array(
				'type'    => 'radio',
				'value'   => 'category',
			...
			'categories' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'category', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary categories', 'freebook' ),
				'multiple'         => true,
				'placeholder'      => esc_html__( 'Select categories', 'freebook' ),
				'master'           => 'terms_type_category',
			),
			'current_tag' => array(
				'type'             => 'select',
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select tag', 'freebook' ),
				'placeholder'      => esc_html__( 'Select tag', 'freebook' ),
			...
			'tags' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary tags', 'freebook' ),
				'multiple'         => true,
			...
			'trim_title_chars' => array(
				...
				'min_value'  => 0,
				'step_value' => 1,
				'label'      => esc_html__( 'Title chars trimmed count', 'freebook' ),
			),
			...
			'current_title_visibility' => array(
				...
				'label' => esc_html__( 'Display current navigation title', 'freebook' ),
			),
			'date_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display date', 'freebook' ),
			),
			'author_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display author', 'freebook' ),
			),
			'comment_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display comments', 'freebook' ),
			),
	...
	public function get_query_items( $query_args = array() ) {
		...
		$defaults_query_args = apply_filters( 'freebook_news_smart_box_default_query_args', array(
			'posts_per_page' => '5',
			'post_type'      => 'post',
			'orderby'        => 'date',
			'order'          => 'DESC',
	...
	public function get_smart_box_loop( $posts_query ) {
		...
		if ( ! $posts_query->have_posts() ) {
			return '<div class="container"><h4>' . esc_html__( 'Posts not found', 'freebook' ) . '</h4></div>';
		}
...
add_action( 'widgets_init', 'freebook_register_news_smart_box_widgets' );
/**
 * Register news-smart-box widget.
 */
function freebook_register_news_smart_box_widgets() {
	register_widget( 'FreeBook_News_Smart_Box_Widget' );
}
