...
class FreeBook_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'freebook' );
		$this->widget_id          = 'freebook_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'freebook' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'freebook' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'freebook' ),
				'label' => esc_html__( 'Subscribe Title', 'freebook' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'freebook' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'freebook' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'freebook' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'freebook' ),
				'label' => esc_html__( 'Subscribe submit label', 'freebook' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'freebook' ),
				'label' => esc_html__( 'Subscribe success', 'freebook' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'freebook' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'freebook' ),
				'label' => esc_html__( 'Follow Title', 'freebook' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'freebook' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'freebook' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'freebook' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'freebook' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'freebook' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'freebook' ),
				'value'   => 'center',
				'options' => array(
					'top-left'      => esc_html__( 'Top Left', 'freebook' ),
					'top-center'    => esc_html__( 'Top Center', 'freebook' ),
					'top-right'     => esc_html__( 'Top Right', 'freebook' ),
					'center-left'   => esc_html__( 'Middle Left', 'freebook' ),
					'center'        => esc_html__( 'Middle Center', 'freebook' ),
					'center-right'  => esc_html__( 'Middle Right', 'freebook' ),
					'bottom-left'   => esc_html__( 'Bottom Left', 'freebook' ),
					'bottom-center' => esc_html__( 'Bottom Center', 'freebook' ),
					'bottom-right'  => esc_html__( 'Bottom Right', 'freebook' ),
				),
				'master' => 'background_image',
			),
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'freebook' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'freebook' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'freebook' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'freebook' ),
				'value'   => 'inherit',
				'options' => array(
					'cover'   => esc_html__( 'Cover', 'freebook' ),
					'contain' => esc_html__( 'Contain', 'freebook' ),
					'auto'    => esc_html__( 'Auto', 'freebook' ),
				),
				'master' => 'background_image',
			),
			'background_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'freebook' ),
				'value'  => '#000000',
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_freebook_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_freebook_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'freebook' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'freebook' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'freebook' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'freebook' ),
		);

		$this->nonce = wp_create_nonce( 'freebook-subscribe-nonce' );
	...
	public function widget( $args, $instance ) {
		...
		$follow_template            = locate_template( apply_filters( 'freebook_subscribe_widget_follow_template_file', 'inc/widgets/subscribe-follow/view/follow-view.php' ), false, false );
		$subscribe_template         = locate_template( apply_filters( 'freebook_subscribe_widget_subscribe_template_file', 'inc/widgets/subscribe-follow/view/subcribe-view.php' ), false, false );
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'freebook' );
		}
	...
	public function get_social_nav() {
		return freebook_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = freebook_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'freebook-subscribe-nonce' ) ) {
...
add_action( 'widgets_init', 'freebook_register_subscribe_follow_widgets' );
/**
 * Register subscribe-follow widget.
 */
function freebook_register_subscribe_follow_widgets() {
	register_widget( 'FreeBook_Subscribe_Follow_Widget' );
}
