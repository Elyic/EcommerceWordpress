...
if ( ! class_exists( 'FreeBook_Playlist_Slider_Widget' ) ) {

	/**
	 * Class FreeBook_Playlist_Slider_Widget.
	 */
	class FreeBook_Playlist_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Playlist Slider', 'freebook' );
			$this->widget_description = esc_html__( 'Display playlist slider on your site.', 'freebook' );
			$this->widget_id          = apply_filters( 'freebook_playlist_slider_widget_ID', 'freebook_widget_playlist_slider' );
			$this->widget_cssclass    = apply_filters( 'freebook_playlist_slider_widget_cssclass', 'widget-playlist-slider' );
			$this->utility            = freebook_utility()->utility;
			$this->settings = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Playlist Slider', 'freebook' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 0,
					'label'     => esc_html__( 'Posts count ( Set 0 to show all. )', 'freebook' ),
				),
				'width' => array(
					'type'  => 'text',
					'value' => '100%',
					'label' => esc_html__( 'Slider width ( px, %, rem )', 'freebook' ),
				),
				'height' => array(
					'type' => 'text',
					'value' => '500',
					'label' => esc_html__( 'Slider height ( px, rem )', 'freebook' ),
				),
				'thumbnail_controls' => array(
					...
					'value'   => true,
					'options' => array(
						false => esc_html__( 'Hide', 'freebook' ),
						true  => esc_html__( 'Buttons', 'freebook' ),
					),
					'label'       => esc_html__( 'Select Thumbnail Controls', 'freebook' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'freebook' ),
				),
				'slider_controls' => array(
					...
					'label'       => esc_html__( 'Select Slider Controls', 'freebook' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'freebook' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Title words length ( Set 0 to hide title. )', 'freebook' ),
				),
				'thumb_title_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Thumbnail title words length ( Set 0 to hide title. )', 'freebook' ),
				),
				'mate_data' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'date'     => esc_html__( 'Date', 'freebook' ),
						'author'   => esc_html__( 'Author', 'freebook' ),
						'comments' => esc_html__( 'Comment count', 'freebook' ),
						'category' => esc_html__( 'Category', 'freebook' ),
						'post_tag' => esc_html__( 'Tag', 'freebook' ),
					),
					'label' => esc_html__( 'Display post meta data', 'freebook' ),
				),
				'thumb_mate_data' => array(
					...
					'label' => esc_html__( 'Display thumbnail post meta data', 'freebook' ),
				),
			);
			...
			$this->layout_settings = apply_filters( 'freebook_playlist_slider_size', array(
		...
		public function the_thumbnail_view() {
			...
			foreach ( $this->posts as $post ) {
				...
				$image = $this->utility->media->get_image( array(
					'size'        => 'freebook-thumb-s',
					'mobile_size' => 'freebook-thumb-s',
		...
		public function widget( $args, $instance ) {
			...
			if ( $this->posts ) {
				$slider_settings = array(
					...
					'buttons'            => apply_filters( 'freebook_playlist_buttons', false ),
...
add_action( 'widgets_init', 'freebook_register_playlist_slider_widget' );
/**
 * Register playlist-slider widget.
 */
function freebook_register_playlist_slider_widget() {
	register_widget( 'FreeBook_Playlist_Slider_Widget' );
}
