...
if ( ! class_exists( 'FreeBook_Custom_Posts_Widget' ) ) {

	/**
	 * Class FreeBook_Custom_Posts_Widget.
	 */
	class FreeBook_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Custom Posts', 'freebook' );
			$this->widget_description = esc_html__( 'Display custom posts your site.', 'freebook' );
			$this->widget_id          = apply_filters( 'freebook_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass    = apply_filters( 'freebook_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility            = freebook_utility()->utility;
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Custom Posts', 'freebook' ),
				...
				'terms_type' => array(
					...
					'options' => array(
						...
						'post_format'   => array(
							'label' => esc_html__( 'Post Format', 'freebook' ),
				...
				'post_format' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select post format', 'freebook' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select post format', 'freebook' ),
				...
				'post_offset' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Offset post', 'freebook' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					'value'      => '10',
				...
				'excerpt_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'freebook' ),
				),
				'meta_data' => array(
					'type'    => 'checkbox',
					'value'   => array(
					...
					'options' => array(
						...
						'comment_count' => esc_html__( 'Comment count', 'freebook' ),
						'category'      => esc_html__( 'Category', 'freebook' ),
						'post_tag'      => esc_html__( 'Tag', 'freebook' ),
						'more_button'   => esc_html__( 'More Button', 'freebook' ),
					),
				...
				'button_text' => array(
					'type'  => 'text',
					'value' => 'Read More',
					'label' => esc_html__( 'Post read more button label', 'freebook' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$template = locate_template( apply_filters( 'freebook_custom_posts_widget_view_dir', 'inc/widgets/custom-posts/views/custom-post-view.php' ), false, false );
			...
			$footer_columns = get_theme_mod( 'footer_widget_columns', freebook_theme()->customizer->get_default( 'footer_widget_columns' ) );
	...
	add_action( 'widgets_init', 'freebook_register_custom_posts_widget' );
	/**
	 * Register custom-posts widget.
	 */
	function freebook_register_custom_posts_widget() {
		register_widget( 'FreeBook_Custom_Posts_Widget' );
	}
}
