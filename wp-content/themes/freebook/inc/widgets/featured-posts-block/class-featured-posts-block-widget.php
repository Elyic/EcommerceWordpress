...
if ( ! class_exists( 'FreeBook_Featured_Posts_Block_Widget' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class FreeBook_Featured_Posts_Block_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Featured Posts Block', 'freebook' );
			$this->widget_description = esc_html__( 'This widget displays latest posts', 'freebook' );
			$this->widget_id          = 'freebook_widget_featured_posts_block';
			...
			foreach( $layouts as $id => $layout ) {
				$layout_options[ $id ] = array(
					'label'   => $layout['name'],
					'img_src' => FREEBOOK_THEME_URI . '/assets/images/admin/widgets/featured-posts-block/' . $id . '.svg',
				);
			}
			...
			$this->settings = array(
				...
				'posts_ids' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Posts IDs (Optional)', 'freebook' ),
				),
				'checkboxes'     => array(
					'type'  => 'checkbox',
					'label' => esc_html__( 'Post Meta', 'freebook' ),
					'value' => array(
						'title'      => 'true',
						'excerpt'    => 'true',
						'categories' => 'true',
					...
					'options' => array(
						'title'      => esc_html__( 'Show title', 'freebook' ),
						'excerpt'    => esc_html__( 'Show excerpt', 'freebook' ),
						'categories' => esc_html__( 'Show categories', 'freebook' ),
						'tags'       => esc_html__( 'Show tags', 'freebook' ),
						'author'     => esc_html__( 'Show author', 'freebook' ),
						'date'       => esc_html__( 'Show date', 'freebook' ),
					),
				),
				'title_length' => array(
					'type'      => 'stepper',
					'value'     => 12,
					'min_value' => 1,
					'label'     => esc_html__( 'Title length (chars)', 'freebook' ),
				),
				'excerpt_length' => array(
					'type'      => 'stepper',
					'value'     => 15,
					'min_value' => 1,
					'label'     => esc_html__( 'Excerpt length (words)', 'freebook' ),
				),
			);
		...
		public function render_layout( $options = array() ) {
			...
			$item_template = locate_template(  apply_filters( 'freebook_featured_posts_block_widget_view_dir', 'inc/widgets/featured-posts-block/views/item.php' ), false, false );
			 ...
			 */
			$query = apply_filters( 'freebook_featured_posts_block_query', $query, $this->instance );
		...
		public function get_layouts() {
			return array(
				'layout-1' => array(
					'name'   => esc_html__( 'Type #1', 'freebook' ),
					'amount' => 5,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-2' => array(
					'name'   => esc_html__( 'Type #2', 'freebook' ),
					'amount' => 5,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-3' => array(
					'name'   => esc_html__( 'Type #3', 'freebook' ),
					'amount' => 4,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-4' => array(
					'name'   => esc_html__( 'Type #4', 'freebook' ),
					'amount' => 4,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-5' => array(
					'name'   => esc_html__( 'Type #5', 'freebook' ),
					'amount' => 3,
					'markup' => array(
						array(
							'type'  => 'item',
	...
	add_action( 'widgets_init', 'freebook_register_featured_posts_block_widget' );

	if ( false === function_exists( 'freebook_register_featured_posts_block_widget' ) ) {
		/**
		 * Register featured posts block widget.
		 */
		function freebook_register_featured_posts_block_widget() {
			register_widget( 'FreeBook_Featured_Posts_Block_Widget' );
		}
	}
}
