 ...
 * @package FreeBook
 */
if ( ! class_exists( 'FreeBook_Image_Grid_Widget' ) ) {

	/**
	 * Image Grid Widget
	 */
	class FreeBook_Image_Grid_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Image Grid', 'freebook' );
			$this->widget_description = esc_html__( 'This widget displays images from post.', 'freebook' );
			...
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => 'Image Grid',
					'label' => esc_html__( 'Widget title', 'freebook' ),
				),
				'terms_type' => array(
					'type'    => 'radio',
					'value'   => 'category_name',
				...
				'category_name' => array(
					...
					'label'            => esc_html__( 'Select categories to show', 'freebook' ),
				...
				'tag' => array(
					...
					'label'            => esc_html__( 'Select tags to show', 'freebook' ),
				...
				'post_sort' => array(
					...
					'value'   => 'date',
					'options' => array(
						'date'          => esc_html__( 'Publish Date', 'freebook' ),
						'title'         => esc_html__( 'Post Title', 'freebook' ),
						'comment_count' => esc_html__( 'Comment Count', 'freebook' ),
					),
					'label'   => esc_html__( 'Post sorted', 'freebook' ),
				),
				'post_number' => array(
					'type'       => 'stepper',
					'value'      => '5',
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Posts number', 'freebook' ),
				),
				'post_offset' => array(
					'type'       => 'stepper',
					'value'      => '0',
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number', 'freebook' ),
				),
				'items_padding' => array(
					'type'       => 'stepper',
					'value'      => '5',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Items padding ( size in pixels )', 'freebook' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$args = apply_filters( 'freebook_image_grid_widget_args', $args, $instance );
			...
			if ( array_key_exists( $terms_type, $instance ) ) {
				...
				if ( $post_taxonomy ) {
					$post_args = array(
						...
						'order'       => apply_filters( 'freebook_order_image_grid_widget', 'DESC', $instance ), // ASC
			...
			if ( isset( $posts ) && $posts ) {
				...
				$grid_class_array = apply_filters( 'freebook_image_grid_widget_grid_class', array(
				...
				echo apply_filters(
					'freebook_image_grid_widget_before',
				...
				echo apply_filters( 'freebook_image_grid_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'freebook_register_image_grid_widget' );
	/**
	 * Register image grid widget.
	 */
	function freebook_register_image_grid_widget() {
		register_widget( 'FreeBook_Image_Grid_Widget' );
	}
}
